...
register_supernode() {
  if is_sn_registered_active; then
    ...
    if [[ -n "$REG_TX_HASH" && "$REG_TX_HASH" != "null" ]]; then
      wait_for_tx "$REG_TX_HASH" || { echo "[SN] Registration tx failed/timeout"; exit 1; }
    else
      echo "[SN] Failed to obtain txhash for registration"; exit 1
    fi
    if is_sn_registered_active; then
...
wait_for_height_at_least 5 || { echo "[SN] Lumera chain not producing blocks in time; exiting."; exit 1; }
