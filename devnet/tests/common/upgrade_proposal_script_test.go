...
func runUpgradeProposalScriptScenario(t *testing.T, testScript string) {
	t.Helper()
...
`
	runUpgradeProposalScriptScenario(t, testScript)
}

func TestUpgradeProposalPrefersPersistedMigrationOverRestoredLegacyKey(t *testing.T) {
	const testScript = `
source "$1"
GOV_ADDRESS_FILE="$(mktemp)"
trap 'rm -f "$GOV_ADDRESS_FILE"' EXIT
...
key_address() {
  [[ "$1" == "governance_key" ]] && printf '%s\n' 'lumera1legacy'
}
account_exists() { [[ "$1" == "lumera1legacy" || "$1" == "lumera1migrated" ]]; }
key_name_for_address() { return 0; }
registry_mnemonic() { printf '%s\n' 'fixture mnemonic'; }
governance_mnemonic_file() { printf '%s\n' '/does/not/exist'; }
recover_evm_key_from_mnemonic() {
...
func TestUpgradeProposalUsesExistingKeyForPersistedAddress(t *testing.T) {
	const testScript = `
source "$1"
GOV_ADDRESS_FILE="$(mktemp)"
trap 'rm -f "$GOV_ADDRESS_FILE"' EXIT
...
key_address() { return 0; }
account_exists() { [[ "$1" == "lumera1migrated" ]]; }
key_name_for_address() { printf '%s\n' 'already_recovered'; }
recover_evm_key_from_mnemonic() { return 1; }

resolve_proposer
[[ "$PROPOSER_KEY_NAME" == "already_recovered" ]]
[[ "$PROPOSER_ADDRESS" == "lumera1migrated" ]]
`
	runUpgradeProposalScriptScenario(t, testScript)
}
