...
// infrastructureLegacyKeyCandidates enumerates per-host single-sig keys that
// aren't generated by prepare's fixture loop but still qualify for migration
// (they hold balance and have a mnemonic persisted in the shared status
// registry). Dynamic names — e.g. the multisig bootstrap funder — are
// discovered via findLocalMultisigValidator so we don't hardcode val2.
func infrastructureLegacyKeyCandidates() []string {
	candidates := []string{
		"governance_key",
		"sncli-account",
	}
...
// that isn't in the local keyring, already tracked in af, missing from the
// status registry, or that resolves to an address already absent on-chain.
func recordInfrastructureLegacyAccounts(af *AccountsFile, existingByName map[string]int) {
	log.Println("--- Recording infrastructure legacy accounts ---")
	for _, name := range infrastructureLegacyKeyCandidates() {
		...
		if err != nil {
			log.Printf("  WARN: %s exists in keyring but getAddress failed: %v", name, err)
			continue
		}
		...
		bal, balErr := queryBalance(addr)
		if balErr != nil && !isAccountNotFoundErr(balErr) {
			log.Printf("  WARN: balance query for %s failed: %v", name, balErr)
		}
		rec := AccountRecord{
			Name:       name,
			Address:    addr,
		...
		idx := len(af.Accounts) - 1
		existingByName[name] = idx
		existingByName[addr] = idx
		log.Printf("  recorded infrastructure legacy account %s: %s balance=%d", name, addr, bal)
	}
}
