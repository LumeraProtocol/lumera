...
Working tree has BRIDGE documentation updates from 2026-05-05 and one remaining non-doc local modification, `devnet/go.mod`, on master. No new in-flight feature branches beyond the merged v1.12.0 cleanup.

## Version Milestones

### v1.12.0 (LEP-5 + Everlight Phase 1) — In Progress (functionally complete, latest re-gate passing)
...
| F17 | S14 | Block Reward Share Routing | Removed completely; no x/distribution block reward split is planned in this project | removed |
...
| Devnet E2E | PASS | `make devnet-tests-everlight` operator-confirmed green on 2026-05-05 after local devnet was available |
...
| Last gate | **PASS** on 2026-05-01 (commit 451f8a8), devnet evidence confirmed 2026-05-05 | 0 blockers, warnings remain scoped/waived; previous run 2026-04-28 was PASS |

Current local worktree drift noted during this update:

- `devnet/go.mod`
- BRIDGE docs updated by this session: `docs/context.json`, `docs/requirements.json`, `docs/STATUS.md`, `docs/gates-evals/S10-S15-gate-report.md`

The earlier regenerated proto/openapi/buf.lock drift was reported reset by the operator. Confirm whether `devnet/go.mod` is intentional before final approval.
...
- ~~Devnet smoke (`make devnet-tests-everlight`) unavailable for the 2026-05-01 re-gate~~ — resolved 2026-05-05 by operator-confirmed green local devnet run.
...
`requirements.json.execution.open_questions` is empty and `context.json.discrepancies` is empty. The items below are remaining decisions/follow-ups surfaced from `docs/decisions.md` and gate-history notes.

| ID | Topic | Status | Notes |
|----|-------|--------|-------|
| OQ-A | Pool-account model: Phase 1 uses the existing `supernode` module account as the Everlight pool. | resolved | Code is canonical. `requirements.json` was reconciled on 2026-05-05 to document the embedded-supernode account model instead of a separate permissionless `everlight` account. |
| OQ-B | F17 / AT40 block reward routing | resolved | Removed completely per operator confirmation on 2026-05-05; no x/distribution AllocateTokens modification is planned in this project. |
...
- **Tag the Everlight Phase 1 release** once final operator approval is given, then drive IAT01–IAT04 across `supernode`, `sdk-go`, `sdk-js`.
