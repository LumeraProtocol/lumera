...
**Endowment is not required for MVP.** The pool funded by Foundation transfers, registration fee share, and Community Pool governance transfers provides a viable starting baseline. Endowments are added in Phase 3, once payout measurement and audit rails are stable.
...
**Key property:** Payouts are funded by Foundation transfers, registration fee share, Community Pool transfers, and (later) endowment staking yield. Users are never billed again after registration.
...
Foundation transfers + registration fee share + Community Pool transfers (and later endowment yield) exceed aggregate storage costs. Data persists indefinitely. Hardware costs historically decline; additional funding streams added in later phases strengthen sustainability.
...
| **Diversified funding** | Multiple revenue sources reduce single-point-of-failure risk (up to 4 across all phases) |
...
| **Low-risk rollout** | Foundation bridge starts payouts before upgrade; Community Pool always available |
...
| Implementation complexity | Higher (3 phases) | Lower (fee + timer) |
| Macro risk exposure | Diversified across up to 4 streams (added incrementally) | None (user pays market rate) |
