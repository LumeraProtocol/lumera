...
| 3.7 | Raise block gas limit via governance           | DEFERRED | 25M is adequate for launch; increase for heavy DeFi if needed |
...
| 5.6 | Production CORS origin lockdown                   | DONE   | `app/openrpc/http.go` — reuses `[json-rpc] ws-origins` for `/openrpc.json` CORS                             |
| 5.7 | JSON-RPC namespace exposure lockdown per env      | DONE   | `cmd/lumera/cmd/jsonrpc_policy.go` — reject `debug`, `personal`, `admin` on `lumera-mainnet*`        |
...
### Integration Tests (115 tests — DONE)
...
| 11.18 | Precompiles (+ gas metering + action module)                                 | 21    | DONE   |
...
| 11.24 | EVMigration tool (prepare, estimate, migrate, migrate-validator, cleanup, verify) | 6 modes | DONE   |
...
## Phase 12: Custom Lumera Module Precompiles (IN PROGRESS)

EVM contracts calling Lumera-specific functionality. Other chains (Evmos, Kava) ship these at launch.
Address convention: `0x0901`–`0x09XX` for Lumera-custom precompiles, above cosmos-evm's `0x0800`–`0x0806` range.

| #    | Item                                     | Status | Priority | Notes                                                                                          |
| ---- | ---------------------------------------- | ------ | -------- | ---------------------------------------------------------------------------------------------- |
| 12.1 | Action precompile (full — read + write)  | DONE   | Medium   | `precompiles/action/` — typed Cascade/Sense request/finalize + generic approve + all queries |
| 12.2 | Action precompile integration tests      | DONE   | Medium   | `tests/integration/evm/precompiles/action_test.go` — getParams, getActionFee, list queries  |
| 12.3 | Action precompile app wiring             | DONE   | Medium   | `app/evm.go`, `app/evm/precompiles.go` — address `0x0901`                                |
| 12.4 | Supernode precompile (full — read + write) | DONE | Medium   | `precompiles/supernode/` — register/deregister/start/stop/update/reportMetrics + all queries |
| 12.5 | Supernode precompile integration tests     | DONE | Medium   | `tests/integration/evm/precompiles/supernode_test.go` — getParams, listSuperNodes, getTopSuperNodesForBlock |
| 12.6 | Supernode precompile app wiring            | DONE | Medium   | `app/evm.go`, `app/evm/precompiles.go` — address `0x0902`                                  |
...
| 14.2 | CORS origin lockdown per environment     | DONE   | High               | `app/openrpc/http.go` — reuses `[json-rpc] ws-origins` for `/openrpc.json` CORS                   |
| 14.3 | JSON-RPC namespace exposure profiles     | DONE   | High               | `cmd/lumera/cmd/jsonrpc_policy.go` — mainnet startup guard rejects `debug`/`personal`/`admin` |
| 14.4 | Fee market monitoring runbook            | TODO   | High               | Base fee tracking, gas utilization, alerting thresholds                                                |
| 14.5 | Node operator EVM configuration guide    | DONE   | High               | `docs/evm-integration/node-evm-config-guide.md`                                                       |
...
| 14    | Production Hardening       | TODO        | 3/8               |
...
2. **Production JSON-RPC hardening** (Phase 14.2) — CORS lockdown remains
3. **Block explorer** (Phase 15.1) — user-facing ecosystem requirement
4. **Monitoring runbook** (Phase 14.4) — operator readiness

### Near-Term Priorities

1. CosmWasm + EVM interaction design (Phase 13.1)
2. Multi-validator EVM consensus testing (Phase 11.30)

### Can Wait

1. External indexer / SDK examples (Phase 15.4-15.5)
