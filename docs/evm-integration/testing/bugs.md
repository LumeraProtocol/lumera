...
### 24) `PermissionlessRegistration` enabled by default allows anyone to register ERC20 token pairs
...
**Symptom**: After migrating a validator that was ever slashed (downtime or double-sign), every reward withdrawal, delegation, undelegation, or redelegation touching a re-keyed delegation fails: the SDK panics with `calculated final stake for delegator ... greater than current stake` inside `calculateDelegationRewards`. The same failure hits an account migration's re-keyed delegation to any ever-slashed validator. A slashed-then-unjailed validator passes the migration pre-checks (only currently-jailed validators are rejected), so the migration itself succeeds and the damage surfaces later.

**Root cause**: `MigrateValidatorDelegations` (validator step V4) and `migrateActiveDelegations` (account path) rebuilt `DelegatorStartingInfo` with `Stake: del.Shares`. The SDK's `initializeDelegation` stores `validator.TokensFromSharesTruncated(shares)` — the delegation's token value. For a validator with slash history the tokens/shares exchange rate is < 1, so raw shares overstate the stake; the SDK's defensive final-stake check (`stake > currentStake` beyond a 3-smallest-dec margin) then panics on the delegator's next withdrawal or share-modifying tx, permanently blocking them for every re-keyed delegator.

**Fix** (`x/evmigration/keeper/migrate_validator.go`, `x/evmigration/keeper/migrate_staking.go`): Both re-key paths now read the target validator record and store `validator.TokensFromSharesTruncated(del.Shares)`, matching the SDK's own starting-info semantics. Validator migration reads the record once (post-V2, at the new operator address); account migration reads it once per re-keyed delegation's validator.

**Tests**: `TestMigrateValidatorDelegations_SlashedValidatorUsesTokenStake`, `TestMigrateStaking_SlashedValidatorUsesTokenStake` — both pin `Stake == tokens-from-shares` for a 10%-slashed validator (900 tokens / 1000 shares), where storing raw shares would fail the assertion.
