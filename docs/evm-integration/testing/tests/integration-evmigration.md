...
| `TestMigrateValidator_RepairsLegacyRawShareStartingInfo` | End-to-end regression for the v1.20.0 raw-share `DelegatorStartingInfo` bug: slashes a real validator, injects the exact legacy fingerprint (Stake = raw shares, starting height past the slash event) on the external delegation, then migrates against real staking/distribution keepers. Without the repair, `WithdrawDelegationRewards` panics in the SDK's `CalculateDelegationRewards`; with it, migration succeeds and the starting stake is corrected to the token value. The self-delegation is left untouched (its slash event post-dates its starting height, so normal SDK replay handles it). |
