...
| Precompile coverage (10 precompiles + gas metering + action + supernode + wasm modules)   | 30+ integration                         | Smoke-level                               |
...
| **Unit**        | x/evmigration                        | 116                                                                     | Excellent — auth/bank/staking/distribution/authz/feegrant/supernode/claim/action migration, validator migration (including evidence, metrics stale-key deletion, account history, third-party withdraw addr preservation, operator cross-validator delegation re-keying), chain-ID domain-separated signature verification (incl. chain-ID mismatch error diagnostics), genesis, queries, params, message validation, rate limiting, pre-checks |
|                       |                                      |                                                                         |                                                                                                                                                                                                                                                                                     |
...
| **Integration** | evm/precompiles                      | 30                                                                      | High — bank, staking, distribution, gov, bech32, p256, slashing (params, signing infos, unjail), ICS20 (denoms, denomHash, denom), action (getParams, getActionFee, getActionsByState, getActionsByCreator, requestCascade bad-sig, approveAction non-existent), supernode (getParams, listSuperNodes, getTopSuperNodesForBlock, register tx, reportMetrics tx, reportMetrics auth), delegate tx, withdraw address tx, gas metering accuracy (6 precompiles), estimate-vs-actual, wasm `eth_call` write-path rejection |
...
|                       |                                      |                                                                         |                                                                                                                                                                                                                                                                                     |
|                       | **Totals**                     | **Unit: ~255 · Integration: ~128 · Devnet: 12+ · Total: ~420** |                                                                                                                                                                                                                                                                                     |
...
| `TestVerifyNewSignature_ChainIDMismatch`                       | Signs new proof with wrong chain ID, verifies address-mismatch error includes chain ID hint (ECDSA recovery succeeds but recovers wrong address).        |
...
| `TestClaimLegacyAccount_RequiresExactlyTwoArgs`            | Verifies claim command requires exactly 2 positional args (legacy-key, new-key).                                          |
| `TestMigrateValidator_RequiresExactlyTwoArgs`              | Verifies validator command requires exactly 2 positional args (legacy-validator-key, new-validator-evm-key).              |
...
| `TestGasAdjustment_DefaultOverriddenTo1_5`                 | Guards against SDK default gas adjustment (1.0) — ensures our `<= 1.0` override condition stays correct.                 |
...
| `WasmPrecompileExecuteEmitsEvent`                          | Verifies `WasmExecuted` EVM log is emitted with correct topic and decodable payload containing the contract address and non-empty response bytes. |
