...
Functional tests, code generation, scoped vulnerability scanning, and `make build` passed. The operator explicitly waived scoped Go lint findings on 2026-05-01. The only blocker was the unavailable local devnet target; the operator later confirmed `make devnet-tests-everlight` was green on 2026-05-05, so this gate is now recorded as PASS.
...
- Everlight devnet target: `make devnet-tests-everlight` - PASS; operator-confirmed green run on 2026-05-05 after local devnet was available
...
| F17 | AT40 | Block reward share routing | Removed from requirements and code; no x/distribution integration planned | REMOVED |
...
## Warnings
1. Worktree was dirty during the original gate: `buf.lock`, `devnet/go.mod`, `docs/static/openapi.yml`, `x/action/v1/types/metadata.pb.go`, `x/supernode/v1/types/params.pb.go`, and `x/supernode/v1/types/supernode_state.pb.go`. Operator reported generated-file drift was reset afterward.
...
## Recommended Actions
1. Keep the operator-confirmed `make devnet-tests-everlight` run as the devnet evidence for this re-gate.
2. Confirm the post-reset worktree only contains intentional local changes before final approval.
3. Optionally clean up the waived unused-symbol lint findings before final merge.
4. Proceed from this PASS gate only after deciding the next BRIDGE slice or closure step.
