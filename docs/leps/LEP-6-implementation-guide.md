...
```text
(target, bucket, ticket_id, artifact_class, artifact_key, artifact_ordinal)
```
...
1. For each reporter reliability state, load reporter result facts in the divergence window.
2. If fact stats are empty, fall back to the reporter state's window counters.
3. Skip reporters with total reports below minimum.
4. Compute `negative_rate = negative_count / total_count`.
5. Compute the median negative rate across qualifying reporters.
6. Penalize a reporter if `negative_rate > 2 * median_negative_rate`.
7. Skip the penalty if at least half of the reporter's negative results were confirmed by recheck.
...
Import/export handles all storage-truth score states, heal ops, and the next heal-op id.
...
Non-e2e verification last run on the LEP-6 activation branch:
...
- `./x/audit/v1/...` passed
- `./tests/integration/audit/...` passed
- `./tests/system/audit/...` passed
