...
var (
	...
	fd_CascadeMetadata_rq_ids_oti protoreflect.FieldDescriptor
...
func init() {
	...
	fd_CascadeMetadata_rq_ids_oti = md_CascadeMetadata.Fields().ByName("rq_ids_oti")
...
func (x *fastReflection_CascadeMetadata) Range(f func(protoreflect.FieldDescriptor, protoreflect.Value) bool) {
	...
	if len(x.RqIdsOti) != 0 {
		value := protoreflect.ValueOfBytes(x.RqIdsOti)
		if !f(fd_CascadeMetadata_rq_ids_oti, value) {
			return
		}
	}
	if x.Signatures != "" {
...
func (x *fastReflection_CascadeMetadata) Has(fd protoreflect.FieldDescriptor) bool {
	...
	case "lumera.action.CascadeMetadata.rq_ids_oti":
		return len(x.RqIdsOti) != 0
...
func (x *fastReflection_CascadeMetadata) Mutable(fd protoreflect.FieldDescriptor) protoreflect.Value {
	...
	case "lumera.action.CascadeMetadata.rq_ids_oti":
		panic(fmt.Errorf("field rq_ids_oti of message lumera.action.CascadeMetadata is not mutable"))
	case "lumera.action.CascadeMetadata.signatures":
...
func (x *fastReflection_CascadeMetadata) ProtoMethods() *protoiface.Methods {
	...
	marshal := func(input protoiface.MarshalInput) (protoiface.MarshalOutput, error) {
		...
		if len(x.RqIdsOti) > 0 {
			i -= len(x.RqIdsOti)
			copy(dAtA[i:], x.RqIdsOti)
			i = runtime.EncodeVarint(dAtA, i, uint64(len(x.RqIdsOti)))
			i--
			dAtA[i] = 0x32
		}
		if len(x.RqIdsIds) > 0 {
	...
	unmarshal := func(input protoiface.UnmarshalInput) (protoiface.UnmarshalOutput, error) {
		...
		for iNdEx < l {
			...
			case 6:
				if wireType != 2 {
					return protoiface.UnmarshalOutput{NoUnkeyedLiterals: input.NoUnkeyedLiterals, Flags: input.Flags}, fmt.Errorf("proto: wrong wireType = %d for field RqIdsOti", wireType)
				}
				var byteLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				x.RqIdsOti = append(x.RqIdsOti[:0], dAtA[iNdEx:postIndex]...)
				if x.RqIdsOti == nil {
					x.RqIdsOti = []byte{}
				}
				iNdEx = postIndex
...
func (x *CascadeMetadata) GetRqIdsOti() []byte {
	if x != nil {
		return x.RqIdsOti
	}
	return nil
