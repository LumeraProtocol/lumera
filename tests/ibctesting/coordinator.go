...
func SameWasmOpts(n int, opts []wasmkeeper.Option) [][]wasmkeeper.Option {
	s := make([][]wasmkeeper.Option, n)
	for i := range s {
		s[i] = opts
	}
	return s
...
func NewCoordinator(t *testing.T, n int, wasmOpts ...[]wasmkeeper.Option) *Coordinator {
	t.Helper()
...
// function expects the channels to be successfully opened otherwise testing will
// fail.
