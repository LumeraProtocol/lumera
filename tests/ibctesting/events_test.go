...
import (
	...
	mockv1 "github.com/LumeraProtocol/lumera/tests/ibctesting/mock"
...
func TestParseV1PacketsFromEvents(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			allPackets, err := ibctst.ParseIBCV1Packets(channeltypes.EventTypeSendPacket, tc.events)
			...
			firstPacket, err := ibctst.ParseV1PacketFromEvents(tc.events)
...
func TestParseV2PacketsFromEvents(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			packets, err := ibctst.ParseIBCV2Packets(tc.eventType, tc.events)
			if tc.expectedError != "" {
				require.ErrorContains(t, err, tc.expectedError)
				return
			}
