...
type TestingApp interface {
	...
	GetIBCRouter() *ibcporttypes.Router
...
func SetupTestingApp(tb testing.TB, wasmOpts ...wasmkeeper.Option) (TestingApp, map[string]json.RawMessage) {
	db := dbm.NewMemDB()
	wasmHome := tb.TempDir()
	appOpts := simtestutil.AppOptionsMap{
		app.WasmHomeDirOption: wasmHome,
	}
	lumeraApp := app.New(log.NewNopLogger(), db, nil, true, appOpts, wasmOpts)
	return lumeraApp, lumeraApp.DefaultGenesis()
}
...
func SetupWithGenesisValSet(
	...
	tb.Helper()
	app, genesisState := DefaultTestingAppInit(tb, wasmOpts...)
