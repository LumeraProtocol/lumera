...
func VoteAndCheckProposalStatus(endpoint *Endpoint, proposalID uint64) error {
	...
	require.NoError(endpoint.Chain.TB, endpoint.Chain.GetLumeraApp().GovKeeper.AddVote(ctx, proposalID, endpoint.Chain.SenderAccount.GetAddress(), govtypesv1.NewNonSplitVoteOption(govtypesv1.OptionYes), ""))

	// fast forward the chain context to end the voting period
	params, err := endpoint.Chain.GetLumeraApp().GovKeeper.Params.Get(ctx)
	...
	p, err := endpoint.Chain.GetLumeraApp().GovKeeper.Proposals.Get(endpoint.Chain.GetContext(), proposalID)
	require.NoError(endpoint.Chain.TB, err)
