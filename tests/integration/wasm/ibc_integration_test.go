...
func TestIBCReflectContract(t *testing.T) {
	...
	initMsg := []byte(`{}`)
	codeID := chainA.StoreCodeFile(GetTestDataFilePath("ibc_reflect_send.wasm")).CodeID
	sendContractAddr := chainA.InstantiateContract(codeID, initMsg)

	reflectID := chainB.StoreCodeFile(GetTestDataFilePath("reflect_1_5.wasm")).CodeID
	initMsg = wasmKeeper.IBCReflectInitMsg{
		ReflectCodeID: reflectID,
	}.GetBytes(t)
	codeID = chainB.StoreCodeFile(GetTestDataFilePath("ibc_reflect.wasm")).CodeID
...
func TestOnChanOpenInitVersion(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			myContract := &ibcmock.MockIBCContractCallbacks{
...
func TestOnIBCPacketReceive(t *testing.T) {
	...
	specs := map[string]struct {
		...
		"all good": {
			...
			expAck:     []byte(`{"ok":{"account":"lumera1suhgf5svhu4usrurvxzlgn54ksxmn8gljarjtxqnapv8kjnp4nrs3x5z8z"}}`),
		},
		"with result err": {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			var (
				coord  = ibctesting.NewCoordinator(t, 2, chainAOpts)
...
func TestIBCAsyncAck(t *testing.T) {
	...
	err = path.RelayPacketWithoutAck((*chainA.PendingSendPackets)[0])
	...
	res, err := chainB.SendMsgs(&wasmtypes.MsgExecuteContract{
	...
	require.NoError(t, err)
	acknowledgement, err := ibctst.ParseAckFromEvents(res.GetEvents())
	require.NoError(t, err)
...
func (x *captureAckTestContractEngine) SubmitIBCPacket(t *testing.T, path *ibctesting.Path, chainA *ibctesting.TestChain, senderContractAddr sdk.AccAddress, packetData []byte) *[]byte {
	t.Helper()
	...
	// start the process
	_, err := chainA.SendMsgs(&wasmtypes.MsgExecuteContract{
