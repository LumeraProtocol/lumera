...
var (
	addrAcc1 = func() sdk.AccAddress {
		_, addr := keyPubAddr()
		return addr
	}()
	addr1        = addrAcc1.String()
	testContract = mustLoad(GetTestDataFilePath("hackatom.wasm"))
	maskContract = testdata.ReflectContractWasm()
	oldContract  = mustLoad(GetTestDataFilePath("escrow_0.7.wasm"))
)
