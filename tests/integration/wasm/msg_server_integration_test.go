...
var reflectContract = mustLoad(GetTestDataFilePath("reflect_1_5.wasm"))
...
func TestStoreCode(t *testing.T) {
	...
	msg := types.MsgStoreCodeFixture(func(m *types.MsgStoreCode) {
		m.WASMByteCode = reflectContract
	...
	expHash, err := wasmvm.CreateChecksum(reflectContract)
	require.NoError(t, err)
...
func TestAddCodeUploadParamsAddresses(t *testing.T) {
	...
	var (
		myAddress    sdk.AccAddress = make([]byte, types.ContractAddrLen)
		otherAddr                   = func() sdk.AccAddress { _, addr := keyPubAddr(); return addr }()
		govAuthority                = wasmApp.WasmKeeper.GetAuthority()
	)
...
func TestStoreAndMigrateContract(t *testing.T) {
	...
	checksum, err := wasmvm.CreateChecksum(testContract)
	require.NoError(t, err)
