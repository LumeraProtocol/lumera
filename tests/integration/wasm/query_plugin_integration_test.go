...
func TestReflectGrpcQuery(t *testing.T) {
	...
	cdc := MakeEncodingConfig(t).Codec
	ctx, keepers := CreateTestInput(t, false, ReflectCapabilities, wasmKeeper.WithMessageEncoders(reflectEncoders(cdc)), wasmKeeper.WithQueryPlugins(&queryPlugins))
	keeper := keepers.WasmKeeper
...
func TestDistributionQuery(t *testing.T) {
	cdc := MakeEncodingConfig(t).Codec
	pCtx, keepers := CreateTestInput(t, false, ReflectCapabilities, wasmKeeper.WithMessageEncoders(reflectEncoders(cdc)), wasmKeeper.WithQueryPlugins(reflectPlugins()))
	keeper := keepers.WasmKeeper
...
func TestAcceptListStargateQuerier(t *testing.T) {
	lumeraApp := app.SetupWithEmptyStore(t)
	ctx := lumeraApp.NewUncachedContext(false, cmtproto.Header{ChainID: "foo", Height: 1, Time: time.Now()})
	err := lumeraApp.StakingKeeper.SetParams(ctx, stakingtypes.DefaultParams())
	require.NoError(t, err)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			q := wasmKeeper.AcceptListStargateQuerier(accepted, lumeraApp.GRPCQueryRouter(), lumeraApp.AppCodec())
