	...
	var (
		chainAOpts = []wasmkeeper.Option{
			wasmkeeper.WithWasmEngine(
				ibcmock.NewIBCContractMockWasmEngine(pingContract)),
		}
		chainBOpts = []wasmkeeper.Option{wasmkeeper.WithWasmEngine(
			ibcmock.NewIBCContractMockWasmEngine(pongContract),
		)}
...
var _ ibcmock.IBCContractCallbacks = &player{}
