...
func TestFromIBCTransferToContract(t *testing.T) {
	...
	specs := map[string]struct {
		contract                    ibcmock.IBCContractCallbacks
		setupContract               func(t *testing.T, contract ibcmock.IBCContractCallbacks, chain *ibctesting.TestChain)
		...
		t.Run(name, func(t *testing.T) {
			var (
				chainAOpts = []wasmkeeper.Option{wasmkeeper.WithWasmEngine(
					ibcmock.NewIBCContractMockWasmEngine(spec.contract),
				)}
				coordinator = ibctesting.NewCoordinator(t, 2, []wasmkeeper.Option{}, chainAOpts)
			...
			err = path.RelayAndAckPendingPackets()
			if spec.expErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestContractCanInitiateIBCTransferMsg(t *testing.T) {
	...
	var (
		chainWasmOpts = []wasmkeeper.Option{
			wasmkeeper.WithWasmEngine(
				ibcmock.NewIBCContractMockWasmEngine(myContract)),
		}
		coordinator = ibctesting.NewCoordinator(t, 2, chainWasmOpts)
		chainA      = coordinator.GetChain(ibctesting.GetChainID(1))
		chainB      = coordinator.GetChain(ibctesting.GetChainID(2))
	)
	...
	// and dest chain balance contains voucher
	bankKeeperB := chainB.GetLumeraApp().BankKeeper
...
func TestContractCanEmulateIBCTransferMessage(t *testing.T) {
	...
	var (
		...
		coordinator = ibctesting.NewCoordinator(t, 2, chainAOpts)
...
func TestContractCanEmulateIBCTransferMessageWithTimeout(t *testing.T) {
	...
	err = ibctesting.TimeoutPendingPackets(coordinator, path)
	require.NoError(t, err)
...
func TestContractEmulateIBCTransferMessageOnDiffContractIBCChannel(t *testing.T) {
	...
	var (
		chainAOpts = []wasmkeeper.Option{
			wasmkeeper.WithWasmEngine(
				ibcmock.NewIBCContractMockWasmEngine(myContractA1),
			),
			wasmkeeper.WithWasmEngine(
				ibcmock.NewIBCContractMockWasmEngine(myContractA2),
			),
		}
...
func TestContractHandlesChannelClose(t *testing.T) {
	...
	var (
		chainAOpts = []wasmkeeper.Option{
			wasmkeeper.WithWasmEngine(
				ibcmock.NewIBCContractMockWasmEngine(myContractA)),
		}
		chainBOpts = []wasmkeeper.Option{
			wasmkeeper.WithWasmEngine(
				ibcmock.NewIBCContractMockWasmEngine(myContractB)),
		}
	...
	path.SetupConnections()
	path.CreateChannels()
	ibctesting.CloseChannel(coordinator, path)
...
func TestContractHandlesChannelCloseNotOwned(t *testing.T) {
	...
	closeIBCChannelMsg := &wasmtypes.MsgExecuteContract{
		Sender:   chainA.SenderAccount.GetAddress().String(),
...
var _ ibcmock.IBCContractCallbacks = &captureCloseContract{}
...
var _ ibcmock.IBCContractCallbacks = &sendViaIBCTransferContract{}
...
var _ ibcmock.IBCContractCallbacks = &sendEmulatedIBCTransferContract{}
...
var _ ibcmock.IBCContractCallbacks = &closeChannelContract{}
...
func (g closeIBCChannel) GetBytes() wasmtypes.RawContractMessage {
...
func (g startTransfer) GetBytes() wasmtypes.RawContractMessage {
	b, err := json.Marshal(g)
	if err != nil {
		panic(err)
	}
...
var _ ibcmock.IBCContractCallbacks = &ackReceiverContract{}
...
func (c *ackReceiverContract) IBCPacketReceive(_ wasmvm.Checksum, _ wasmvmtypes.Env, msg wasmvmtypes.IBCPacketReceiveMsg, _ wasmvm.KVStore, _ wasmvm.GoAPI, _ wasmvm.Querier, _ wasmvm.GasMeter, _ uint64, _ wasmvmtypes.UFraction) (*wasmvmtypes.IBCReceiveResult, uint64, error) {
	...
	err := c.chain.GetLumeraApp().TransferKeeper.OnRecvPacket(ctx, srcV2, ibcPacket.SourcePort, ibcPacket.SourceChannel, ibcPacket.DestinationPort, ibcPacket.DestinationChannel)
	if err != nil {
...
func (c *ackReceiverContract) IBCPacketAck(_ wasmvm.Checksum, _ wasmvmtypes.Env, msg wasmvmtypes.IBCPacketAckMsg, _ wasmvm.KVStore, _ wasmvm.GoAPI, _ wasmvm.Querier, _ wasmvm.GasMeter, _ uint64, _ wasmvmtypes.UFraction) (*wasmvmtypes.IBCBasicResult, uint64, error) {
	...
	err := c.chain.GetLumeraApp().TransferKeeper.OnAcknowledgementPacket(ctx, ibcPacket.SourcePort, ibcPacket.SourceChannel, dataV2, ack)
	if err != nil {
		return nil, 0, errorsmod.Wrap(err, "within our smart contract")
	}
