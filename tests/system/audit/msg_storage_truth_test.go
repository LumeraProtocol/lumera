...
func TestSubmitStorageRecheckEvidence_AccumulatesAcrossTickets(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		// Each recheck has a distinct transcript hash (recheck hashes must be unique per 122-F3).
		recheckHash := "hash-recheck-" + string(rune('1'+i))
		...
		req := &types.MsgSubmitStorageRecheckEvidence{
			...
			RecheckTranscriptHash:          recheckHash,
			RecheckResultClass:             types.StorageProofResultClass_STORAGE_PROOF_RESULT_CLASS_RECHECK_CONFIRMED_FAIL,
		}
		_, err := s.msgServer.SubmitStorageRecheckEvidence(s.ctx, req)
		require.NoError(t, err)
	...
	// 3 × RECHECK_CONFIRMED_FAIL base delta (15 each) = 45.
	// Pattern escalation bonus is NOT applied for RECHECK bucket (121-F1).
	require.Equal(t, int64(45), state.SuspicionScore)
}
