...
func TestICA(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			contApp := controllerChain.App.(*pApp.App)
...
func parseIBCChannelEvents(t *testing.T, res *abci.ExecTxResult) (string, string, string) {
	t.Helper()
	chanID, err := pibctesting.ParseChannelIDFromEvents(res.GetEvents())
	require.NoError(t, err)
	portID, err := pibctesting.ParsePortIDFromEvents(res.GetEvents())
	require.NoError(t, err)
	version, err := pibctesting.ParseChannelVersionFromEvents(res.GetEvents())
	require.NoError(t, err)
