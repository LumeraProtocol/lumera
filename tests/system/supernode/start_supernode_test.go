...
func TestStartSupernode(t *testing.T) {
	...
	newStartSupernodeMsg := func(creator string, validator string) *types.MsgStartSupernode {
		return &types.MsgStartSupernode{
			Creator:          creator,
	...
	testCases := []struct {
		...
		verify func(t *testing.T, suite *SystemTestSuite, resp *types.MsgStartSupernodeResponse, err error)
	}{
		{
			...
			verify: func(t *testing.T, suite *SystemTestSuite, resp *types.MsgStartSupernodeResponse, err error) {
				...
				require.Equal(t, types.SuperNodeStateActive, sn.States[len(sn.States)-1].State)

				// Check for event
				events := suite.sdkCtx.EventManager().Events()
				var foundStartEvent bool
				for _, evt := range events {
					if evt.Type == types.EventTypeSupernodeStarted {
						foundStartEvent = true
						for _, attr := range evt.Attributes {
							if string(attr.Key) == types.AttributeKeyValidatorAddress {
