...
func TestStoreCodeGrant(t *testing.T) {
	reflectWasmCode, err := wasmtest.ReadTestContractFile("reflect_1_1.wasm")
	require.NoError(t, err)
...
func TestGzipStoreCodeGrant(t *testing.T) {
	hackatomWasmCode, err := wasmtest.ReadTestContractFile("hackatom.wasm")
	require.NoError(t, err)

	hackatomGzipWasmCode, err := wasmtest.ReadTestContractFile("hackatom.wasm.gzip")
	require.NoError(t, err)
...
func TestBrokenGzipStoreCodeGrant(t *testing.T) {
	brokenGzipWasmCode, err := wasmtest.ReadTestContractFile("broken_crc.gzip")
	require.NoError(t, err)
