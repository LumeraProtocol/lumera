...
func TestIBC2SendReceiveMsg(t *testing.T) {
	...
	ibc2FilePath := wasmtest.GetTestDataFilePath("ibc2.wasm")
	contractCodeA := chainA.StoreCodeFile(ibc2FilePath).CodeID
	...
	contractCodeB := chainB.StoreCodeFile(ibc2FilePath).CodeID
	...
	err = path.RelayPendingPacketsV2()
	require.NoError(t, err)
...
func TestIBC2TimeoutMsg(t *testing.T) {
	...
	err = ibctesting.TimeoutPendingPacketsV2(coord, path)
	require.NoError(t, err)
