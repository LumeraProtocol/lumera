...
func TestIBCCallbacks(t *testing.T) {
	...
	ibcCallbacksFilePath := wasmtest.GetTestDataFilePath("ibc_callbacks.wasm")
	// with an ibc-callbacks contract deployed on chain A
	codeIDonA := chainA.StoreCodeFile(ibcCallbacksFilePath).CodeID

	// and on chain B
	codeIDonB := chainB.StoreCodeFile(ibcCallbacksFilePath).CodeID
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			if spec.expAck {
				...
				path.RelayAndAckPendingPackets()
				...
				require.NoError(t, ibctesting.TimeoutPendingPackets(coord, path))
...
func TestIBCCallbacksWithoutEntrypoints(t *testing.T) {
	...
	contractAddrA := wasmtest.InstantiateReflectContract(t, chainA)
	chainA.Fund(contractAddrA, oneToken.Amount)
	contractAddrB := wasmtest.InstantiateReflectContract(t, chainA)

	// when the contract on A sends an IBCMsg::Transfer to the contract on B
	memo := fmt.Sprintf(`{"src_callback":{"address":"%v"},"dest_callback":{"address":"%v"}}`, contractAddrA.String(), contractAddrB.String())
	wasmtest.MustExecViaReflectContract(t, chainA, contractAddrA, wasmvmtypes.CosmosMsg{
