...
func (k *Keeper) RegisterAction(ctx sdk.Context, action *actionapi.Action) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(types.ErrInvalidActionType, err.Error())
	}

	// Call the handler's RegisterAction method with the metadata
	err = handler.RegisterAction(ctx, action)
	if err != nil {
		return "", errors.Wrap(types.ErrInvalidMetadata, err.Error())
	}
	...
	if err != nil {
		return "", errors.Wrap(types.ErrInternalError, err.Error())
	}
	...
	if action.State == actionapi.ActionState_ACTION_STATE_PENDING {
		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				types.EventTypeActionRegistered,
				sdk.NewAttribute(types.AttributeKeyActionID, action.ActionID),
				sdk.NewAttribute(types.AttributeKeyCreator, action.Creator),
				sdk.NewAttribute(types.AttributeKeyActionType, action.ActionType.String()),
				sdk.NewAttribute(types.AttributeKeyFee, action.Price),
			),
		)
	}
...
func (k *Keeper) FinalizeAction(ctx sdk.Context, actionID string, superNodeAccount /*creator!*/ string, newMetadata []byte) error {
	...
	if err != nil {
		return errors.Wrap(types.ErrInternalError, fmt.Sprintf("failed to update action: %v", err))
	}
	...
	if existingAction.State == actionapi.ActionState_ACTION_STATE_FAILED {
		...
		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				types.EventTypeActionFailed,
				sdk.NewAttribute(types.AttributeKeyActionID, existingAction.ActionID),
				sdk.NewAttribute(types.AttributeKeyCreator, existingAction.Creator),
				sdk.NewAttribute(types.AttributeKeyActionType, existingAction.ActionType.String()),
				sdk.NewAttribute(types.AttributeKeyError, "finalization failed"),
				sdk.NewAttribute(types.AttributeKeySuperNodes, strings.Join(existingAction.SuperNodes, ",")),
			),
		)
		return errors.Wrapf(types.ErrFinalizationError, "action %s failed", actionID)
	}
	...
	if existingAction.State == actionapi.ActionState_ACTION_STATE_DONE {
		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				types.EventTypeActionFinalized,
				sdk.NewAttribute(types.AttributeKeyActionID, existingAction.ActionID),
				sdk.NewAttribute(types.AttributeKeyCreator, existingAction.Creator),
				sdk.NewAttribute(types.AttributeKeyActionType, existingAction.ActionType.String()),
				sdk.NewAttribute(types.AttributeKeySuperNodes, strings.Join(existingAction.SuperNodes, ",")),
...
func (k *Keeper) DistributeFees(ctx sdk.Context, actionID string) error {
	...
	for sn := range uniqueSupernodes {
		...
		err = k.bankKeeper.SendCoinsFromModuleToAccount(
			ctx,
			types.ModuleName,       // Module account name
			snAddr,                 // Recipient
