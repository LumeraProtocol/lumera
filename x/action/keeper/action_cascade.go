...
func (h CascadeActionHandler) Process(metadataBytes []byte, msgType common.MessageType, params *types.Params) ([]byte, error) {
...
func (h CascadeActionHandler) RegisterAction(ctx sdk.Context, action *actionapi.Action) error {
	...
	if err := proto.Unmarshal(action.Metadata, &cascadeMeta); err != nil {
		return errors.Wrap(types.ErrInvalidMetadata, fmt.Sprintf("failed to unmarshal cascade metadata: %v", err))
	}
	...
	if len(signatureParts) != 2 {
		return errors.Wrap(types.ErrInvalidMetadata, "invalid signature format")
	}
	...
	if err := h.keeper.VerifySignature(ctx, dataToVerify, creatorSignature, action.Creator); err != nil {
		return errors.Wrap(types.ErrInvalidMetadata, fmt.Sprintf("failed to verify creator's signature: %v", err))
	}
	return nil
}
...
func (h CascadeActionHandler) FinalizeAction(ctx sdk.Context, action *actionapi.Action, superNodeAccount string, metadataBytes []byte) (actionapi.ActionState, error) {
	...
	if existingCascadeMeta.FileName == "" {
		return actionapi.ActionState_ACTION_STATE_UNSPECIFIED,
			errors.Wrap(types.ErrInvalidMetadata, "file_name is required in existing metadata")
	}
	if existingCascadeMeta.RqIdsIc == 0 {
		return actionapi.ActionState_ACTION_STATE_UNSPECIFIED,
			errors.Wrap(types.ErrInvalidMetadata, "rq_ids_ic is required in existing metadata")
	}
	if existingCascadeMeta.RqIdsMax == 0 {
		return actionapi.ActionState_ACTION_STATE_UNSPECIFIED,
			errors.Wrap(types.ErrInvalidMetadata, "rq_ids_max is required in existing metadata")
	}
	if len(existingCascadeMeta.Signatures) == 0 {
		return actionapi.ActionState_ACTION_STATE_UNSPECIFIED,
			errors.Wrap(types.ErrInvalidMetadata, "signatures is required in existing metadata")
	}

	var newCascadeMeta actionapi.CascadeMetadata
	if err := proto.Unmarshal(metadataBytes, &newCascadeMeta); err != nil {
		return actionapi.ActionState_ACTION_STATE_UNSPECIFIED,
			errors.Wrap(types.ErrInvalidMetadata, fmt.Sprintf("failed to unmarshal cascade metadata: %v", err))
	}
	...
	if err := VerifyKademliaIDs(newCascadeMeta.RqIdsIds, existingCascadeMeta.Signatures, existingCascadeMeta.RqIdsIc, existingCascadeMeta.RqIdsMax); err != nil {
		return actionapi.ActionState_ACTION_STATE_UNSPECIFIED,
			errors.Wrap(types.ErrInvalidMetadata, fmt.Sprintf("failed to verify rq_ids_ids: %v", err))
	}
