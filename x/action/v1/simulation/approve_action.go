...
func SimulateMsgApproveActionSuccess(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		if err != nil {
			return simtypes.NoOpMsg(types2.ModuleName, sdk.MsgTypeURL(&types2.MsgApproveAction{}), "failed to get random supernodes"), nil, nil
		}

		// 3. Create approval message
		msg := types2.NewMsgApproveAction(
		...
		if !found {
			return simtypes.NoOpMsg(types2.ModuleName, sdk.MsgTypeURL(msg), "action not found after approval"), nil, nil
		}

		if approvedAction.State != actionapi.ActionState_ACTION_STATE_APPROVED {
			return simtypes.NoOpMsg(types2.ModuleName, sdk.MsgTypeURL(msg), "action not in APPROVED state after approval"), nil, nil
		}
...
func SimulateMsgApproveActionInvalidState(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		if !found {
			return simtypes.NoOpMsg(types2.ModuleName, sdk.MsgTypeURL(msg), "action not found after attempt"), nil, nil
		}

		if unchangedAction.State != action.State {
			return simtypes.NoOpMsg(types2.ModuleName, sdk.MsgTypeURL(msg), "action state changed unexpectedly"), nil, nil
		}
...
func SimulateMsgApproveActionUnauthorized(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		if err == nil {
			return simtypes.NoOpMsg(types2.ModuleName, sdk.MsgTypeURL(msg), "expected unauthorized error but got none"), nil, nil
		}
