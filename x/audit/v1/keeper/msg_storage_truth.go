...
func (m msgServer) SubmitStorageRecheckEvidence(ctx context.Context, req *types.MsgSubmitStorageRecheckEvidence) (*types.MsgSubmitStorageRecheckEvidenceResponse, error) {
	...
	// doesn't permanently block the submitter from retrying (121-Copilot-1).
	// Per 122-Copilot-1 + Roomote-B — module-error wrapped.
	// linkStorageTruthRecheckTranscript uses errorsmod.Wrapf internally; pass through.
	...
	// Capture the original reporter from the challenged transcript record (121-F2).
	// Using challengedRecord.ReporterAccount is authoritative: it is the reporter who
	// originally submitted the challenged result, not the last reporter to touch the ticket.
...
func (m msgServer) ClaimHealComplete(ctx context.Context, req *types.MsgClaimHealComplete) (*types.MsgClaimHealCompleteResponse, error) {
	...
	sdkCtx.EventManager().EmitEvent(
		sdk.NewEvent(
			...
			sdk.NewAttribute(types.AttributeKeyHealManifestHash, req.HealManifestHash),
		),
	)
...
func (m msgServer) SubmitHealVerification(ctx context.Context, req *types.MsgSubmitHealVerification) (*types.MsgSubmitHealVerificationResponse, error) {
	...
	// Per 120-F6 — positive attestations must commit to the canonical result hash.
	if req.Verified && healOp.ResultHash != "" && req.VerificationHash != healOp.ResultHash {
		return nil, errorsmod.Wrapf(types.ErrInvalidHealVerification,
			"positive attestation hash %x mismatches heal-op result hash %x",
			req.VerificationHash, healOp.ResultHash)
	}
