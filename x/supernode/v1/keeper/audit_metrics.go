...
func (k Keeper) getLatestCascadeBytesFromAudit(ctx sdk.Context, supernodeAccount string) (float64, int64, bool) {
	auditKeeper := k.auditKeeper
	if auditKeeper == nil {
		auditKeeper = globalAuditKeeper
	}
	if auditKeeper == nil || supernodeAccount == "" {
		return 0, 0, false
	}
	...
	for offset := uint64(0); offset <= maxAuditEpochLookback && offset <= currentEpochID; offset++ {
		...
		report, found := auditKeeper.GetReport(ctx, epochID, supernodeAccount)
		if !found {
			continue
		}
