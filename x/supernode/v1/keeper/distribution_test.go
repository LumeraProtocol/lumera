...
func setupTestKeeper(t *testing.T) (Keeper, sdk.Context, *mockBankKeeper, *mockSupernodeKeeper, *mockAuditKeeper) {
	...
	snKeeper := newMockSupernodeKeeper(&k, ctx)
...
func addSupernode(snKeeper *mockSupernodeKeeper, auditKeeper *mockAuditKeeper, valAddr sdk.ValAddress, accAddr sdk.AccAddress, state sntypes.SuperNodeState, cascadeBytes float64) {
	...
	sn := sntypes.SuperNode{
		...
		Note:             sntypes.DefaultVersion,
		P2PPort:          sntypes.DefaultP2PPort,
		PrevIpAddresses:  []*sntypes.IPAddressHistory{{Address: "10.0.0.1", Height: snKeeper.ctx.BlockHeight()}},
		States: []*sntypes.SuperNodeStateRecord{
			{State: state, Height: snKeeper.ctx.BlockHeight()},
		},
	}
...
func TestDistributePoolProportionally(t *testing.T) {
	...
	params.RewardDistribution.NewSnRampUpPeriods = 0 // No ramp-up for this test.
	params.RewardDistribution.MeasurementSmoothingPeriods = 1
	params.RewardDistribution.UsageGrowthCapBpsPerPeriod = 10000 // 100% cap (effectively no cap).
...
func TestMinCascadeBytesThreshold(t *testing.T) {
	...
	params := sntypes.DefaultParams()
	params.RewardDistribution.PaymentPeriodBlocks = 10
	params.RewardDistribution.MinCascadeBytesForPayment = 5000 // 5000 bytes minimum
	params.RewardDistribution.NewSnRampUpPeriods = 0
	params.RewardDistribution.MeasurementSmoothingPeriods = 1
	params.RewardDistribution.UsageGrowthCapBpsPerPeriod = 10000
	require.NoError(t, k.SetParams(ctx, params))
...
func TestNewSNRampUp(t *testing.T) {
	...
	params.RewardDistribution.PaymentPeriodBlocks = 10
	params.RewardDistribution.MinCascadeBytesForPayment = 1000
	params.RewardDistribution.NewSnRampUpPeriods = 4 // 4-period ramp-up
	params.RewardDistribution.MeasurementSmoothingPeriods = 1
	params.RewardDistribution.UsageGrowthCapBpsPerPeriod = 10000
	require.NoError(t, k.SetParams(ctx, params))
...
func TestUsageGrowthCap(t *testing.T) {
	...
	params.RewardDistribution.MinCascadeBytesForPayment = 1000
	params.RewardDistribution.NewSnRampUpPeriods = 0
	params.RewardDistribution.MeasurementSmoothingPeriods = 1   // Near-zero smoothing for clarity.
	params.RewardDistribution.UsageGrowthCapBpsPerPeriod = 1000 // 10% max growth per period.
	require.NoError(t, k.SetParams(ctx, params))
