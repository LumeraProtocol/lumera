...
//  2. SendCoinsFromAccountToModule to the named Everlight pool account works correctly.
//  3. The module account is registered without mint/burn/staking permissions.
...
func TestModuleAccountAddressDeterministic(t *testing.T) {
	...
	addr1 := authtypes.NewModuleAddress(types.EverlightPoolAccountName)
	...
	addrViaKeeper := ak.GetModuleAddress(types.EverlightPoolAccountName)
	...
	require.Equal(t, "everlight", types.EverlightPoolAccountName)
}
...
func TestSendCoinsFromAccountToModule(t *testing.T) {
	...
	err := bk.SendCoinsFromAccountToModule(
		context.Background(), sender, types.EverlightPoolAccountName, amount,
	)
	...
	require.Equal(t, types.EverlightPoolAccountName, transfer.recipientModule)
	require.Equal(t, amount, transfer.amount)

	// Verify balance changes.
	moduleAddr := authtypes.NewModuleAddress(types.EverlightPoolAccountName)
...
// no privileged permissions, matching the named pool-account registration in app_config.go.
func TestModuleAccountPermissions(t *testing.T) {
	ak := &mockAccountKeeperWithPerms{
		permissions: map[string][]string{
			types.EverlightPoolAccountName: {},
		},
	}
	...
	// Verify name.
	require.Equal(t, types.EverlightPoolAccountName, modAcc.GetName())

	// Verify the account has no privileged permissions.
	require.False(t, modAcc.HasPermission(authtypes.Burner),
		"everlight pool account must NOT have Burner permission")
	require.False(t, modAcc.HasPermission(authtypes.Minter),
		"everlight pool account must NOT have Minter permission")
	require.False(t, modAcc.HasPermission(authtypes.Staking),
		"everlight pool account must NOT have Staking permission")

	// Verify the module address on the account matches the deterministic address.
	expectedAddr := authtypes.NewModuleAddress(types.EverlightPoolAccountName)
